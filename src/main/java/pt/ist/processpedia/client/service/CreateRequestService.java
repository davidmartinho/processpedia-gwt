...
public abstract class CreateRequestService extends PostService<RequestDto> {
  ...
  private final CreateRequestInputDto createRequestInputDto;

	public CreateRequestService(CreateRequestInputDto createRequestInputDto) throws CannotExternalizeObjectException {
    super(URL.encode(URLBase.REQUEST_BASE_URL));
		this.createRequestInputDto = createRequestInputDto;
		setRequestData(getTranslator().externalize(createRequestInputDto));
  }

	public CreateRequestInputDto getCreateRequestInputDto() {
		return createRequestInputDto;
	}
